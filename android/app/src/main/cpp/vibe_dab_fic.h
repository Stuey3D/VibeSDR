...
struct Ensemble {
    ...
    bool mciComplete() const {
        ...
        for (const auto& kv : services) {
            if (kv.second.isData) continue;          // ★ never listed; must not hold "complete" open
            ++audio;
            if (kv.second.complete(subChannels)) ++n;
        }
        /* ★ BBC National (12B, measured 2026-09-07) does not transmit FIG 0/7 at all, so without
         *  the count the best available answer is "every audio service we know of is complete".
         *  Driven through the web client the same day, the pane sat on "14 (reading…)" for ever
         *  because the ensemble also carries data services that never get a label. */
        if (serviceCount < 0) return n > 0 && n == audio;
