...
struct AircraftView: View {
  ...
  private let tickClock = Timer.publish(every: 0.5, on: .main, in: .common).autoconnect()
  ...
  private var planes: [Aircraft] {
    link.aircraftList.sorted { a, b in
  ...
  private var header: some View {
    VStack(alignment: .leading, spacing: 5) {
      HStack(spacing: 6) {
        ...
        Text("\(link.aircraftList.count)").font(.system(size: 15, weight: .bold, design: .rounded)).monospacedDigit().foregroundStyle(.white)
  ...
  private var located: [Aircraft] {
    link.aircraftList.filter { $0.lat != nil && $0.lon != nil }
