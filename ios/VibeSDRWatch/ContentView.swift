...
enum LinkHint: Equatable {
  ...
  case reconnecting
  /// First few seconds of a FRESH session: the link meter reads low while it's still coming up. That
  /// is not a poor link, it's a starting one — say "Initialising", never "Server link poor".
  case initialising
...
struct ContentView: View {
  ...
  var body: some View {
    ZStack {
      ...
      /* ★★★ ONE TOP-LEFT COLUMN, USERS ABOVE HARDWARE — they were two overlays at the SAME
       *   coordinates (both `.leading 6 .top 19`) and drew on top of each other. The hardware
       *   button's own comment recorded the assumption that broke it: "(This corner is free on
       *   VibeServer — no chat glyph.)" True when written; then VibeServer grew a shared dial and
       *   brought the chat glyph to a screen that already had the antenna.
       * ★★ Users FIRST because they are what CHANGES — a listener arriving, a message landing, the
       *   dial being armed. The hardware button is a door that is always in the same place.
       * ★ Same fix, same reasoning, as Jr (spike/WristSDR). Keep the two in step. */
      VStack {
        HStack {
          VStack(alignment: .leading, spacing: 4) {
            if link.supportsChat {
              // ★★ Carries the DIAL'S ARM STATE and is the way in to changing it: the arm switch
              //   lives on the chat page on purpose, so reaching for the tuner walks you past the
              //   place you would ask for it.
              ChatGlyph(clients: link.sharedDial ? link.dialListeners : link.clients,
                        activity: link.chatActivity,
                        dialArmed: link.sharedDial ? link.dialArmed : nil) {
    ...
    // ★ No hardware sheet in Buddy: gain/bias-T/AGC/ppm/rate act on the DONGLE, which the phone
    //   owns, and the wire has no commands for them. `hasHardwareControls` is false so the tile
    ...
    .onChange(of: crown) { _, new in
      ...
      // ★ DO NOT REPLACE WITH WKInterfaceVolumeControl. Jr hands the crown to the native HUD,
      //   which drives the WATCH's own output. Buddy's crown drives the PHONE's FULL system volume
      //   via cmd:vol deltas — and the phone mirrors back what it actually applied, including
      //   changes made ON the phone. That is strictly better and must survive the port.
    ...
    // ★ Buddy KEEPS the crown in volume mode. Jr releases SwiftUI focus so the native volume
    //   control can consume it; here the crown must stay ours, because it is what sends cmd:vol
    //   to the phone. Releasing it would leave volume mode with no crown consumer at all.
    .onChange(of: crownMode) { _, _ in crownFocused = true }
  ...
  private var crownOverlay: some View {
    ...
    ZStack {
      ...
      // ★ THE RING IN EVERY MODE, VOLUME INCLUDED. Jr swaps in the native
      //   WKInterfaceVolumeControl here — but that control drives the WATCH's own output, and
      //   Buddy's crown drives the PHONE's system volume. So there is nothing to hand the crown
      //   to, and the ring (which already fills from `link.volume`, the phone's real level) is
      //   both the correct reading and the look being carried over.
      do {
  ...
  private var rawHint: LinkHint? {
    ...
    // ★ SETTLE WINDOW (Stuart): for a second or two after a fresh connect OR a wrist-up, the link
    //    meter reads low while the waterfall stabilises — that's not a poor link, so don't cry
    //    "Server link poor". Say Initialising (fresh session) / Reconnecting (wrist up-down) instead.
    if now.timeIntervalSince(link.resumedAt) < 2.5 { return .reconnecting }
    if let ls = link.liveSince, now.timeIntervalSince(ls) < 2.5 { return .initialising }
  ...
  private func heldLongEnough(_ h: LinkHint, now: Date) -> Bool {
    switch h {
    case .reconnecting, .initialising, .serverHop, .powersave:
      return true
  ...
  private var stalledMessage: (icon: String, text: String)? {
    ...
    // ★ In Buddy the SERVER connection is the phone's, so "we still hold a client" is always
    ...
    .background(alignment: .leading) {
      GeometryReader { geo in
        ...
        ZStack(alignment: .leading) {
          ...
          // Red squelch threshold line (white halo so it reads over any fill colour).
          if link.sql >= 0 {
...
/// Same double-duty readout as the phone — no extra screen space on the wrist.
