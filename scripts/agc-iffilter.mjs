/**
 * agc-iffilter.mjs — the TUNER'S IF FILTER SHAPE, measured against itself.
 *
 *   node scripts/agc-iffilter.mjs ws://host:port 96.1M
 *
 * ★★★ WHY DIFFERENTIALLY. The obvious method — look for where the spectrum falls away at the span
 *     edges — is confounded by whatever is ON AIR there. A strong station near the edge holds the
 *     trace up and the roll-off vanishes: measured at 105.7 (quiet edges) a commanded 800 kHz
 *     showed its -3 dB point 534 kHz in; measured at 96.1 (a blowtorch near the edge) the SAME
 *     setting showed no roll-off whatsoever. I took three readings from the cooperative frequency
 *     and derived a constant from them, which is overfitting to one patch of band.
 * ★★★ SUBTRACTING TWO CAPTURES OF THE SAME FREQUENCY CANCELS THE BAND. Take the spectrum with the
 *     filter WIDE, take it again at the setting under test, and the difference per bin IS the
 *     filter's response — whatever is or is not transmitting. The gain is pinned for both so the
 *     only thing that changes is the filter.
 *
...
(async () => {
  ...
  const rows = [];
  const FIX_GAIN = 250;
  send({ type: 'gain', value: FIX_GAIN });
  send({ type: 'adcstats', seconds: 240 });
  send({ type: 'zoom', frequency: FREQ, binBandwidth: 2400 });
  send({ type: 'tune', frequency: FREQ, mode: 'wfm' });

  const capture = async (bw) => {
    send({ type: 'tunerbw', value: bw });
    await sleep(2500);
    frames = [];
    await sleep(6000);
    if (!frames.length) return null;
    const n = frames[0].length, a = new Float64Array(n);
    for (const f of frames) for (let i = 0; i < n; i++) a[i] += f[i];
    for (let i = 0; i < n; i++) a[i] /= frames.length;
    return a;
  };

  const ref = await capture(0);
  if (!ref) { console.error('no spectrum'); process.exit(1); }
  const n = ref.length, mid = n >> 1;
  console.log(`  ${n} bins x ${(binHz / 1e3).toFixed(1)} kHz, span ${(n * binHz / 1e6).toFixed(2)} MHz`);
  console.log(`  reference: IF wide, gain pinned at ${(FIX_GAIN / 10).toFixed(1)} dB\n`);
  console.log('  ATTENUATION (dB) vs WIDE, at this offset from centre (kHz), mean of both sides');
  console.log('   IF kHz    100   200   300   400   500   600   800  1000');
  for (const bw of [1500000, 1200000, 800000, 600000, 450000, 350000]) {
    const a = await capture(bw);
    if (!a) { console.log(`  ${(bw/1e3).toFixed(0)} kHz — no spectrum`); continue; }
    // ★ The filter's own response: how much this setting removed, bin by bin, relative to wide.
    const drop = new Float64Array(n);
    for (let i = 0; i < n; i++) drop[i] = a[i] - ref[i];
    // ★ Normalise on the middle, where the filter is flat by definition — any overall level shift
    //   between the two captures (AGC twitch, band fade) is common-mode and must not be read as
    //   filtering.
    /* ★★★ SMOOTH BEFORE LOOKING FOR A CROSSING, AND REQUIRE IT TO STAY CROSSED. The difference of
     *     two averaged spectra is still noisy bin to bin, and searching outward from the centre
     *     for the first bin pair 3 dB down finds NOISE almost immediately: it reported a 143 kHz
     *     half-width for a commanded 1500 kHz, and a near-identical ratio for every setting, which
     *     is the signature of a detector measuring its own noise rather than the thing in front of
     *     it. A 21-bin box car and a sustained-crossing test fix both. */
    const sm = new Float64Array(n);
    const W = 10;
    for (let i = 0; i < n; i++) {
      let acc = 0, cnt = 0;
      for (let j = Math.max(0, i - W); j <= Math.min(n - 1, i + W); j++) { acc += drop[j]; cnt++; }
      sm[i] = acc / cnt;
    }
    /* ★★★ PRINT THE CURVE, DO NOT DETECT ON IT. Three different crossing-detectors gave three
     *     different answers here — 143 kHz, then 84 kHz, for settings of 1500 and 800 kHz. When a
     *     detector keeps disagreeing with itself the honest move is to show the measurement and
     *     read it, rather than write a fourth one and believe that. */
    const midDrop = Array.from(sm.slice(mid - 40, mid + 40)).sort((x, y) => x - y)[40];
    const at = (offHz) => {
      const k2 = Math.round(offHz / binHz);
      if (mid + k2 >= n || mid - k2 < 0) return '  -  ';
      const v = ((sm[mid + k2] - midDrop) + (sm[mid - k2] - midDrop)) / 2;
      return v.toFixed(1).padStart(5);
    };
    console.log(`  ${(bw / 1e3).toFixed(0).padStart(5)}  `
      + [100, 200, 300, 400, 500, 600, 800, 1000].map((o) => at(o * 1e3)).join(' '));
  }
