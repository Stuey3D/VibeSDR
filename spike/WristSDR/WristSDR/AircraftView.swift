...
/// ★★★ ONE SCALE FOR OUR OWN CHROME. Watch faces run 324pt (40mm) to 410pt (49mm) — a 27% spread —
/// and every button and glyph was an absolute point size tuned on whichever watch happened to be in
/// hand. So the FM-DX controls fitted the Ultra and fouled the station name on a 44mm, and would be
/// worse again on a 41mm.
///
/// ★★ This is NOT the mistake the clock scrim made (see the note where it used to be). That pill was
/// scaled by screen width while the thing it sat behind — the SYSTEM CLOCK — does not scale, so it
/// drifted off its target. These are OUR controls: they scale with the layout around them, which is
/// exactly what the screen size should drive.
///
/// Anchored at the Ultra 3, so the largest watch keeps the sizes as written (ratio 1.0) and smaller
/// ones step down: 45mm 0.97, 44mm 0.90, 41mm 0.86, 40mm 0.80. Floored so nothing becomes a
/// tap target too small to hit, and never enlarged past the values in the source.
enum WatchScale {
  ...
  /// Round to whole points — half-points on a 1x/2x screen blur a 1pt stroke.
  static func s(_ v: CGFloat) -> CGFloat { (v * k).rounded() }
}

struct QualityGlyph: View {
  @ObservedObject var link: SpikeLink
  /// ★ SIZABLE, and scaled by default. It was a fixed 15pt — the taller half of the method+quality
  ///   pair, so it drove the group's height on every screen regardless of how little room there was.
