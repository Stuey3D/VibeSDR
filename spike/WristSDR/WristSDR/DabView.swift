...
struct DabView: View {
  ...
  var body: some View {
    ...
    /* ★★★ THE OPTIMISTIC LABEL ENDS WHERE THE SERVER AGREES. Cleared only when the confirmed
     *  block IS the one the crown asked for — clearing on any change would hand the capsule back
     *  to a report from the mux we just left, which is the very confusion this replaced. */
    .onChange(of: link.dabBlockName) { _, name in
  ...
  /// The block the capsule should DRAW: what the crown has reached, falling back to what the server
  ...
  private var header: some View {
    VStack(alignment: .leading, spacing: 4) {
       ...
       *  the one line a glance is actually after. It scrolls only when it does not fit. */
