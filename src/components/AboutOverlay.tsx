...
const VERSION_HISTORY: { v: string; detail: string }[] = [
  ...
  { v: 'V10.3.1', detail: 'Audio could go silent on connecting \u2014 fixed, in two places. A change in 10.3 made the start of playback a race with itself, so on some connections the app set the audio path up and then immediately tore it down again: a perfect signal, a moving data meter and no sound, on any server, and often fine on the next attempt. \u2014 Separately, when connecting to an UberSDR receiver by its address on your own network, the app could open its audio connection before the server had registered the session, and the server quietly dropped it; the app then retried for ever without ever fixing it. It now waits for the server, and if the audio does stall it re-introduces itself instead of knocking on a door that will not open. \u2014 The app also keeps its own record of what the audio path did, so a report from Diagnostics says what actually happened rather than what was assumed.' },
  { v: 'V10.3', detail: 'The weak\u2011signal release for broadcast FM, and the controls to go with it. \u2014 A VibeServer running 3.1 now treats FOUR DIFFERENT FAULTS separately, because they want opposite cures: NR takes the hiss off a weak station by rolling the top off the stereo difference signal (FM noise is worst exactly where stereo lives) while keeping the bass and mid image, and it treats mono hiss too; IMS narrows the receiver only when a strong NEIGHBOUR is interfering, and deliberately NOT for noise, where narrowing measurably costs you; CEQ undoes a REFLECTION \u2014 multipath is distortion, not weakness, and a strong station can suffer it; NB removes impulse noise from ignition, thermostats and switch\u2011mode supplies. All four are on by default and each declines to act unless its own measurements say it will help, so a strong station is left exactly as it was. Each has its own switch in the audio menu for A/B\u2011ing what it is doing. \u2014 ADVANCED RDS gained the readings behind them: MPX signal\u2011to\u2011noise with the filter corners it produced, multipath depth (and \u201ctoo noisy to judge\u201d when it honestly cannot tell), what the equaliser achieved before and after, how much the blanker is removing, and whether narrowing the receiver would help or cost. Alternative frequencies are now listed with a TICK against the ones confirmed \u2014 on a noisy station the unconfirmed ones are usually phantoms invented by block errors. \u2014 FM stereo holds on far better: stations that under\u2011inject their pilot (below the 6\u20137.5 kHz specification) now lock and stay locked, and two long\u2011standing faults that dropped stereo for a second at a time \u2014 one on every RDS refresh, one every time the Advanced RDS panel was opened \u2014 are fixed. Measured from on\u2011air recordings: seventeen dropouts became none.' },
