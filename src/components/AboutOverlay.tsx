...
const VERSION_HISTORY: { v: string; detail: string }[] = [
  { v: 'V10.0.2', detail: 'Fixes, all of them things that made a working radio look broken. \u2014 Audio could die on a noisy or half\u2011received station and stay dead until you retuned, while the spectrum, RDS and the signal meter carried on perfectly. One bad packet was stopping the audio decoder and nothing ever restarted it. It now picks itself back up. \u2014 A radio that stopped delivering \u2014 an Airspy nudged on its USB plug, an SDRplay that stalled \u2014 was reported as dead until the server was restarted, even though it was still plugged in and lit. The server now keeps trying to recover it, and keeps telling you the truth while it does. \u2014 Changing the Airspy HF+ sample rate left the radio and the rest of the app disagreeing about what rate it was running at: audio came out too fast or too slow, some rates landed off frequency, and the spectrum lagged behind the sound. Only the highest rate was unaffected. All rates work now. \u2014 Plugging a radio into an Android phone announced \u201cRTL\u2011SDR\u201d whatever you had plugged in. It names the radio you actually connected. \u2014 A VibeServer on a Raspberry Pi now announces itself on the network, so it appears in the server list instead of having to be typed in by IP address.' },
...
const V10_CHANGES: string[] = [
  ...
  'VibeServer: it knows what radio it has. An SDRplay RSP (on the Mac) is offered its LNA state and IF gain reduction with a live total\u2011system\u2011gain readout so you can WATCH the AGC work; an Airspy HF+ gets its attenuator, preamp and AGC and the sample rates the radio itself reports; an RTL\u2011SDR gets its gain table. Controls that a given radio does not have are no longer shown \u2014 a slider that cannot move the receiver is worse than no slider.',
