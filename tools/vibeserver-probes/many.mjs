// N healthy listeners at once — does every one of them actually get served?
import net from 'node:net'; import crypto from 'node:crypto';
const PORT=Number(process.argv[2]), N=Number(process.argv[3]||8), HOST=process.env.HOST||'127.0.0.1';
const n=new Array(N).fill(0), t0=Date.now();
for(let i=0;i<N;i++) setTimeout(()=>{
  const key=crypto.randomBytes(16).toString('base64'); const s=net.connect(PORT,HOST);
  let hs=false,buf=Buffer.alloc(0);
  s.on('connect',()=>s.write(`GET /ws/user-spectrum?user_session_id=L${i}&bins=1024 HTTP/1.1\r\nHost: ${HOST}\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Key: ${key}\r\nSec-WebSocket-Version: 13\r\n\r\n`));
  s.on('error',e=>console.log(`L${i} err ${e.code}`));
...
setTimeout(()=>{
  ...
  console.log(`\nafter ${t}s — frames per listener:`);
  n.forEach((v,i)=>console.log(`  L${i}: ${v}`));
  const live=n.filter(v=>v>50).length;
  console.log(`\n${live}/${N} listeners streaming properly`);
